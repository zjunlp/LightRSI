...
export function applySafetyPolicy(
  ...
  // Decide tool groups first; a group evicts only if strictly closed AND every
  // member item is individually clearable.
  ...
  for (const pair of pairs.values()) {
    ...
    if (isEvictablePair(pair.status) && everyMemberClearable) {
      for (const seq of memberSeqs) action.set(seq, "evict");
    } else {
      for (const seq of memberSeqs) action.set(seq, "keep");
  ...
  for (const item of items) {
    ...
    action.set(item.sourceEventSeq, isClearable(item, effective) ? "evict" : "keep");
  }
  ...
  deferredCallIds.sort();

  return { action, evictSeqs, keepSeqs, deferredCallIds };
}
